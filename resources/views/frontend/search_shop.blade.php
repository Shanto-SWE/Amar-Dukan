...
.search-icon{
   padding-right:40px;
    ...
    <input type="text" id="search-bar" name="search_shop" placeholder="Search Your Shop..."  autocomplete="off">
<button class="search_shop btn btn-warning" type="submit">FIND SHOP</button>
    ...
    <div class="container shop pt-3 pb-5">
      @if($single_name)
    <h4>Search result for {{$shop_name}}!</h4>
    @else
    <h4 class="text-center">Sorry..Search result for store name {{$shop_name}} not found  !</h4>
    @endif
    <div class="row">
        ...
        <div class="col-md-3 mt-2">
           ...
           <div class=" card shopcontent">
             @if((int)$row->open_time <= (int)$currentTime && (int)$row->close_time>=(int)$currentTime)
              <a href="{{route('website.home',$row->shop_slug)}}">
