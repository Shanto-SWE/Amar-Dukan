    ...
    <div class="row ">
        ...
        <div class="col-md-8">
            ...
            <div class="card-header">
                ...
                <h4 style="padding-left:10px;">My Requested Orders</h4>
                   <div>
                       <table class="table">
                         <thead>
                         <tr>
                         ...
                         <tbody>
                         @foreach($requested_orders as $row)
                           <tr>
                             <td scope="row">{{ $row->request_id }}</td>
                             <td>{{ date('d F , Y', strtotime($row->date)) }}</td>
                             <td>{{$row->product_name}}</td>
              
                             <td>
                              @if($row->status==0)
                                 <span class="badge  text-white bg-danger">Pendding</span>
                              @elseif($row->status==1)
                                 <span class="badge text-white  bg-info">Recieved</span>
                              @elseif($row->status==2)
                                 <span class="badge text-white bg-primary">Shipped</span> 
                              @elseif($row->status==3)
                                 <span class="badge text-white bg-success">Completed</span>   
                              @elseif($row->status==4)  
                                 <span class="badge text-white bg-warning">Return</span>
                                 @else
                                 <span class="badge text-white bg-danger">Cancel</span>
                              @endif          
                            </td>
                           ...
                           @endif
                            	<a href="{{route('viewrequest.order',$row->id)}}" class="btn btn-sm btn-info" title="view order"><i class="fa fa-eye"></i></a>
                            </td>
                           </tr>
                          @endforeach 
                         </tbody>
...
$('body').on('click','.requestOrderInvoice', function(){
    ...
    var url = "{{ url('/user/request-order/print') }}/"+id;
